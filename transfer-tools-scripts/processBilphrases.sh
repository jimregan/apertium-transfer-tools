...
zcat $INPUTFILE |  grep -v -F '^*' | grep -v -F '^$' | grep -v -F '<sent>' | grep -F -v '<cm>' | grep  -v '<[rl]par>' | grep  -v '<[rl]quest>' |  sed 's:| *\^:| ':g | sed 's:\$ *|: |:g' | sed 's:\$ *\^: :g' | gzip > $INPUTFILE.clean.gz
zcat $INPUTFILE | sed 's:^[^|]*| ::' | LC_ALL=C sort | uniq -c | sed 's:^ *::' | sed 's:\^:| ^:'  |  grep -v -F '^*' | grep -v -F '^$' | grep -v -F '<sent>' | grep -F -v '<cm>' | grep  -v '<[rl]par>' | grep  -v '<[rl]quest>' |  sed 's:| *\^:| ':g | sed 's:\$ *|: |:g' | sed 's:\$ *\^: :g' | gzip > $INPUTFILE.cleanuniq.gz
...
if [ "$SAMESTRUCTURE" != "" ]; then
  ...
  zcat $INPUTFILE.clean.gz  | paste -d '|' - $INPUTFILE.clean.bildic | python $RULELEARNINGDIR/processBilphrases.py --closed_categories ./markers  $STRUCTPARAMETER  $ENDSALIGNEDFLAG | python $RULELEARNINGDIR/removeBilphraseConflictsWithWordId.py |  sed 's:^[^|]*| ::' | LC_ALL=C sort | uniq -c | sed 's:^ *::' | sed 's:<:| <:' | gzip
else
